...
describe('loadPromptTemplates', () => {
  ...
  it('uses overridden templates verbatim', () => {
    ...
    expect(t.summarySystemPrompt).toBe('tpl-summarySystemPrompt')
  })
})
...
describe('PROMPTS via prompts.yaml', () => {
  ...
  it('renders direct chat with and without memory blocks', async () => {
    ...
    const empty = PROMPTS.DIRECT_CHAT_WITH_TOOLS({ assistantMemory: '', userMemory: '', sharedContext: '' })
    expect(empty).not.toContain('Your persona and behavior')

    const withMem = PROMPTS.DIRECT_CHAT_WITH_TOOLS({
      assistantMemory: 'Call me Boss',
      userMemory: '',
      sharedContext: ''
    })
    expect(withMem).toContain('Your persona and behavior')
  ...
  it('renders summary user prompt with history lines', async () => {
    const { PROMPTS } = await import('../llms/prompts')
    const out = PROMPTS.SUMMARY_USER_PROMPT([
