...
//
// End-to-end test of the clisr router topology with a real local model.
//
// Boots a ClisrServer (router role) + a callback client (client role) wired to a
// real OpenAI-compatible server, then drives ServerLLMProvider through a full
// multi-turn tool loop: pod -> router -> client -> model -> tool_calls ->
// pod executes tool -> resubmits -> final answer.
//
// Disabled by default. Enable with AI_BOT_E2E=1. Requires a local model server.
//
//   AI_BOT_E2E=1 rushx test e2e-clisr-router
//
// Env overrides: AI_BOT_E2E_URL, AI_BOT_E2E_KEY.
//
