...
if [ ! -f "${OUT_DIR}/fake-video.mp4" ]; then
  docker run --rm --entrypoint ffmpeg -v "${OUT_DIR}:/out" "${PREFIX}/stream:${TAG}" \
...
if [ ! -f "${OUT_DIR}/fake-doc.docx" ]; then
  docker run --rm --user root --entrypoint sh -v "${OUT_DIR}:/out" "${PREFIX}/preview:${TAG}" -c '
    set -e
    tmp=$(mktemp -d)
    printf "Sanity preview fixture\n\nGenerated for the docx preview test.\n" > "$tmp/fake-doc.txt"
    HOME="$tmp" libreoffice --headless --convert-to docx:"MS Word 2007 XML" --outdir "$tmp" "$tmp/fake-doc.txt" >/dev/null
