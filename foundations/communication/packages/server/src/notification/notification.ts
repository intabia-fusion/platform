...
import { type Event, MessageEventType } from '@hcengineering/communication-sdk-types'
import {
  BlobID,
  type CardID,
  type CardType,
...
export async function notify (ctx: TriggerCtx, event: Enriched<Event>): Promise<Event[]> {
  switch (event.type) {
    case MessageEventType.CreateMessage: {
      ...
      const meta = await ctx.client.getMessageMeta(event.docClass, event.docId, event.messageId)
      if (meta == null) return []
