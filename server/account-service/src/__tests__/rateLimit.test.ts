...
describe('getClientIp', () => {
  ...
  it('takes the first x-forwarded-for entry when the proxy is trusted', () => {
    expect(getClientIp({ 'x-forwarded-for': '9.9.9.9, 10.0.0.5' }, '10.0.0.1', true)).toBe('9.9.9.9')
  })
  ...
  it('takes the first entry when the header arrives repeated', () => {
    expect(getClientIp({ 'x-forwarded-for': ['9.9.9.9', '8.8.8.8'] }, '10.0.0.1', true)).toBe('9.9.9.9')
  })

  it('keeps an IPv6 address intact', () => {
    expect(getClientIp({ 'x-forwarded-for': '2001:db8::1, 10.0.0.5' }, '10.0.0.1', true)).toBe('2001:db8::1')
  })
