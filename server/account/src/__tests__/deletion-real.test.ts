...
describe.each(realDbFlavors)('deletion-real [$flavor]', ({ flavor, adminUri, dbUri }) => {
  ...
  it('drops the rows that still point at the account row', async () => {
    const uuid = await makeAccount('billing@example.com')
    const ws = await makeWorkspace(uuid, 'active')
    await db.assignWorkspace(uuid, ws, AccountRole.Owner)
    await db.subscription.insertOne({
      workspaceUuid: ws,
      accountUuid: uuid,
      provider: 'manual',
      providerSubscriptionId: `sub-${seq++}`,
      type: SubscriptionType.Tier,
      status: SubscriptionStatus.Active,
      plan: 'rare'
    })
    await db.workspacePermission.insertOne({ workspaceUuid: ws, accountUuid: uuid, permission: 'manage' })

    await db.deleteAccount(uuid)

    expect(await db.account.findOne({ uuid })).toBeNull()
    expect(await db.subscription.find({ workspaceUuid: ws })).toHaveLength(0)
    expect(await db.workspacePermission.find({ workspaceUuid: ws })).toHaveLength(0)
  })
