...
export class TemplatesPage extends CalendarPage {
  ...
  async createTemplate (data: NewTemplate): Promise<void> {
    ...
    if (data.category != null) {
      ...
      await this.page.locator('div.selectPopup').waitFor({ state: 'visible' })
      await this.page
        .locator('div.selectPopup button.menu-item', { hasText: data.category })
        .filter({ hasText: data.category })
        .first()
        .click()
      await this.page.locator('div.selectPopup').waitFor({ state: 'hidden' })
      await expect(this.buttonNewTemplateCategory).toContainText(data.category)
    }
