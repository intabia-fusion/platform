...
>
  <div class="flex-col">
    <div class="mb-2">
      <Label label={login.string.DeleteAccountConfirm} />
    </div>
    <div class="mb-2">
      {#if sendFailed}
        <span class="error-color"><Label label={login.string.ConfirmationFailed} /></span>
      {:else if sent}
        <Label label={login.string.CodeSent} />
      {/if}
    </div>
    <div class="flex-row-center">
      <EditBox bind:value={code} placeholder={login.string.EnterCode} kind={'large-style'} autoFocus />
      <div class="ml-4">
        <Button label={login.string.ResendCode} disabled={sending || retryLeft > 0} on:click={sendCode} />
      </div>
      {#if retryLeft > 0}
