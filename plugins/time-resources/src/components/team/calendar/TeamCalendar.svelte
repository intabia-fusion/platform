...
<script lang="ts">
  ...
  $: if (showActivity && personsSocialIds.length > 0) {
    txCreateQuery.query<TxCUD<Doc>>(
      ...
      // Counters and the tooltip need no tx payload; the limit caps a week of a busy workspace.
      {
        limit: 2000,
  ...
  // groupTeamData scans every person, so calling it per (person x day) cell is quadratic in
  // the row count - group once per day column instead and index the result by person.
  let dayGroups = new Map<Timestamp, Map<Ref<Person>, EventPersonMapping>>()
  $: dayGroups = newDayGroups(allSlots, allEvents, busySlots, todos, $calendarByIdStore)

  function newDayGroups (...deps: unknown[]): Map<Timestamp, Map<Ref<Person>, EventPersonMapping>> {
    return new Map()
  }

  function groupsOf (
    cache: Map<Timestamp, Map<Ref<Person>, EventPersonMapping>>,
    dayFrom: Timestamp,
    dayTo: Timestamp
  ): Map<Ref<Person>, EventPersonMapping> {
    let res = cache.get(dayFrom)
    if (res === undefined) {
      res = new Map(
        groupTeamData(
...
<PersonCalendar persons={personsRefs} startDate={currentDate} {maxDays}>
  <svelte:fragment slot="day" let:day let:today let:weekend let:person let:height>
    ...
    {@const gitem = groupsOf(dayGroups, dayFrom, dayTo).get(person)}
