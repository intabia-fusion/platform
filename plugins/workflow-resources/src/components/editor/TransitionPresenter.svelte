...
<div class="transition-item-presenter" use:tooltip={{ label: getEmbeddedLabel(tooltipText), timeout: 600 }}>
  <span class="transition-name">{transition?.name}</span>
  <span class="colon">:</span>

  <div class="statuses-flow">
    {#if fromStatuses.length === 0}
      <span class="any-status"><Label label={plugin.string.AnyStatus} /></span>
    {:else}
      <div class="status-list">
        {#each fromStatuses as status, i (status._id)}
          {#if i > 0 && showFromNames}<span class="separator">,</span>{/if}
