...
<script lang="ts">
  ...
  import type { Applet, AppletDraft } from '@hcengineering/activity'
...
<Component
  ...
  on:change={() => {
    if (model.createComponent != null) {
      showPopup(model.createComponent, { applet: model, params: applet.params, isEdit: true }, 'center', (result) => {
        if (result != null) {
