...
/**
 * Registers server resource locations required by the tool commands.
 * @public
 */
export function registerToolLocations (): void {
  if (locationsRegistered) return
...
let modelTxes: Tx[] | undefined

/**
 * Building the model takes seconds, so it is built once per process. prepareToolsRaw still clones the
 * txes per call, so callers keep their own copy, and DB_URL is re-read every time - embedders switch
 * it between regions.
 * @public
 */
