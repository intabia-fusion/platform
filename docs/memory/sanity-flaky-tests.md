...
- **Comment counter reads one higher than the database** (`issues.spec` "Add comment by popup",
  3/10 on 2026-09-16). Checked over 24 issues: `issue.comments` always equals the real
  `ChatMessage` count, and the issue from the failing run has 1/1 — so the client's copy of the doc
  is wrong, not the server. The counter is `object.comments`, a collection field the client bumps
  from the `$inc` in `TxUpdateDoc`. `packages/query` already guards this (`isLoadedAtModifiedOn` in
  `handleDocUpdate`, the clone in `getCurrentDoc`); which path still double-applies is unknown.
  **Not reproducible** — 10 idle repeats and 12 under tracker+chat load all passed, so nothing was
  changed in the core package. Next step is a deterministic unit test in
  `foundations/core/packages/query` (findAll in flight + an `$inc` tx).
