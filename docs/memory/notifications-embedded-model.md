...
- Старые covering-индексы `notification_dnc` содержали `INCLUDE data`. После того как карточки и `unread*` переехали в `data`, строка индекса превышала лимит btree (`index row size N exceeds btree version 4 maximum 2704`), и любая запись контекста падала. Индексы дропнуты (`0002_dropOversizedDncIndexes.sql`), их ключи покрыты индексами из 0010. Новые индексы с `INCLUDE data` на этой таблице создавать нельзя.
- Миграции лежат по файлу на флавор: Cockroach не видит колонку, добавленную ранее в том же файле (`42703`, docs/memory/account_db_migrations.md), поэтому для него `ADD COLUMN`, бэкфилл, constraints и индексы - отдельные номера 0001-0005/0006-0008/0009/0010, а у Postgres то же лежит в 0001-0005 и 0010. Мигратор отказывается работать при флаворе `unknown`: применил бы только generic-файлы и поднял версию. `EXPECTED_SCHEMA_VERSION` - счётчик наборов изменений, не файлов.
...
- `applyResult` публикует `queueMessages` уже после применения контекстов, а повторная доставка Tx распознаётся как записанная и ничего не публикует: при отказе продюсера (8 попыток, около минуты) push и email батча теряются без дублей в inbox. Лог `push and email of this batch are lost` - точка алерта; безопасно «додослать» нельзя, не зная, ушла ли первая публикация.
