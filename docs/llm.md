...
Список задач: `docs/llm-tasks.md`. Конфигурация и деплой (роли MODE, env,
реестр провайдеров, топологии): `docs/llm-deploy.md`.

> **ВНИМАНИЕ.** Раздел 2 ниже - ПЕРВОНАЧАЛЬНЫЙ дизайн. По ходу реализации часть
> решений переосмыслена/отброшена. АКТУАЛЬНОЕ состояние:
> - Конфигурация/роли/реестр: `docs/llm-deploy.md`.
> - Что реально сделано + отброшенные решения: `docs/llm-tasks.md` (секции
>   "Отброшенные решения" и "Реальный остаток").
> Ключевые расхождения раздела 2 с реализацией: `AIModelConfig` -> `AIProviderConfig`
> (1 провайдер = N уровней); `AIConversation` -> chunter-треды (T7); `ROUTER_KIND`
> -> роли через `MODE` (T9); pod-side резолв уровня (`LevelResolver`/подписка) ->
> server-trigger кладёт `AILevelSetting.level` в событие; `AIModelInfo`-документ ->
> aibot API `GET /levels`. Раздел оставлен как история проектирования.
