...
- [ ] **Sort comparator typo kills unread-first ordering** — `plugins/chunter-resources/src/components/chat/utils.ts:221`
...
- [ ] **`isTriggerCtx` mutates shared request context** — `foundations/server/packages/middleware/src/triggers.ts:293-295`
...
- [ ] **Stale-tx guard applied to only one entity** — `services/notifications/src/cache.ts:812`
...
- [ ] **Poison message blocks the partition** — `services/notification/pod-notification/src/main.ts:126-129`
...
- [ ] **`serviceTxes` grows unbounded** — `services/notifications/src/cache.ts:233`
...
- [ ] **Stale write in notify-status update** — `plugins/workbench-resources/src/components/Applications.svelte:95-113`
...
- [ ] **Missing required env vars fail late** — `services/notification/pod-notification/src/config.ts:34-49`
...
- [ ] **Missing reactive dependency** — `plugins/chunter-resources/src/components/ReverseChannelScrollView.svelte:381-396`
...
- [ ] **Unread anchor drops notification-level state** — `plugins/chunter-resources/src/chatViewport.ts:553-585`
...
- [ ] **Read accumulator not deduped** — `plugins/chunter-resources/src/scroll.ts:57-67`
...
- [ ] **Collaborator-removal cleanup lost** — `server-plugins/notification-resources/src/index.ts`
...
- [ ] **`jumpToDate` always resets the viewport** — `plugins/chunter-resources/src/chatViewport.ts:341-388`
...
- [ ] `server-plugins/notification/src/index.ts:118` — `PushNotificationsHandler` still declared in the trigger map; implementation (`push.ts`) and model registration are deleted. Dead key, not a crash. (Surfaced independently by two agents.)
- [ ] `models/notification/src/migrations/clear.ts:57-92` — `removeEmptyContexts` deletes contexts with no notifications; if an empty context is valid (pinned chat with no messages), it gets wiped.
...
- [ ] `plugins/notification-resources/src/components/inbox/Inbox.svelte:145-161` — `selectedContext` is taken only from the paginated page (limit 20); a deep link to a context outside the first page never opens the panel.
- [ ] `plugins/notification-resources/src/client.ts:80-94` — `init()` does not clear `contextByDoc` / `contextById` / `readStateByDoc` / `docSettingByDoc` on workspace switch within one tab.
- [ ] `plugins/notification-resources/src/webpush.ts` — no unsubscribe on logout.
- [ ] `plugins/chunter/src/index.ts:263`, `plugins/chunter-resources/src/plugin.ts:46` — Resource types `ShowNotifyMarkerFn` and `GetUnreadThreadsCount` diverge from their implementations (signature and `Promise`). Not broken today, but would hide a real mismatch once a second caller appears.
