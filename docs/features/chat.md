...
- **Server-дедупликация 1:1 DM через referenceId.** SHA-256 отсортированных members, `Forbidden` при повторе; только для DM из 2 участников. - `ChunterMiddleware.onDirectCreate`, `getMembersHash`, server-plugins/chunter/src/middleware.ts; тест server-plugins/chunter/src/__tests__/direct-create.spec.ts.
- **Защита неизменяемости DM.** `referenceId`/`private`/`type` менять нельзя никогда; `members` - нельзя, если у DM есть `referenceId` или `type: 'person'`. - `ChunterMiddleware.onDirectUpdate`.
- **Миграция дублированных DM.** `migrateDuplicatedDirects` сливает старые DM с одинаковым составом, удаляет дубли. - models/chunter/src/migration.ts (state `migrate-duplicated-directs-v1`).
...
- **Reply / Forward / Вложения.** Actions `ReplyToMessage`, `ForwardMessage` -> `ForwardMessageDialog`; `TChatMessage.attachments` (PropCollection `attachment.class.Attachment`). - models/chunter/src/actions.ts; plugins/chunter-resources/src/index.ts.
- **Поиск по сообщениям.** `createChatSearchStore` (курсор, `generation`, `appendUnique`, автодогрузка "тонких" страниц < PAGE_SIZE/2, не более `MAX_AUTO_PAGES=5`). - plugins/chunter-resources/src/search/store.ts.
