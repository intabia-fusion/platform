...
Как настроить подпись кода (code signing) и нотаризацию для desktop-приложения Platform на macOS.
...
После добавления всех 6 secrets в GitHub (Settings → Secrets and variables → Actions) правка `.github/workflows/main.yml` не нужна: job `dist-build` (запускается на теги `v*`/`s*`) уже содержит активные шаги "Install the Apple certificate and provisioning profile" и "Build distribution's" - они читают эти secrets напрямую и подписывают/нотаризуют сборку через `dist-signed` (`desktop-package/package.json`, `desktop-package/scripts/notarize.js`). Секреты подхватятся автоматически на следующем тегированном прогоне.
...
- [features/platform-infra.md](features/platform-infra.md) - Platform-infra фичи, включая desktop Electron.
- [memory/desktop-update-system.md](memory/desktop-update-system.md) - цепочка автообновления Electron-приложения.
